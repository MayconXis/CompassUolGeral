...
*** Test Cases ***
Cenario Manipulacao 01: Criar e Validar JSON Dinamicamente
...
Cenario Manipulacao 02: Modificar Dados JSON Existentes
...
Cenario Manipulacao 03: Extrair Dados de JSON Simulado
    [Documentation]    Extrai campos especificos de um JSON simulado
    [Tags]    json    extracao
    
    ${json_simulado}=    Evaluate    {"data": {"first_name": "Maycon", "email": "maycon@teste.com"}}
    

    ${primeiro_nome}=   Set Variable    ${json_simulado}[data][first_name]
    ${email}=           Set Variable    ${json_simulado}[data][email]
    
    Should Be Equal              ${primeiro_nome}   Maycon
    Should Contain               ${email}           maycon@teste.com
    Log    Dados extraidos: Nome=${primeiro_nome}, Email=${email}

Cenario Manipulacao 04: Trabalhar com Arrays JSON
    [Documentation]    Manipula arrays JSON e processa cada elemento
    [Tags]    json    arrays
    
    @{lista_usuarios}=    Create List
    
    &{usuario1}=    Create Dictionary    name=Usuario Um      email=maycon@qa.com
    &{usuario2}=    Create Dictionary    name=Usuario Dois    email=usuario@qa.com
    &{usuario3}=    Create Dictionary    name=Usuario Tres    email=fulano@qa.com
    ...
    FOR    ${usuario}    IN    @{lista_usuarios}
        ...
        Log    Usuario validado: ${usuario}[name] - ${usuario}[email]
    END

Cenario Manipulacao 05: Validar Estrutura JSON Complexa
